import{l as $}from"./context.CC6GLfpp.js";import{t as b,f as c,x as s,j as S,m,o as _,i as H,r as P}from"./messenger-client.Bks7hfH7.js";import{S as M}from"./signal-watcher.DchcTGV4.js";import{e as I,m as T,o as C,p as A,l as D}from"./styles.BcPDAjHV.js";import{n as E,a as B}from"./live.CKubG-fH.js";import{a as w,c as y,t as j}from"./property.BEi3PmIb.js";import"./chunk.RWPQHXWX.DBFmzxw3.js";import{o as z}from"./message-input.vaoYdxCO.js";import{m as O}from"./context.BqoJQclL.js";import"./tslib.es6.kHcLnhpD.js";var F=Object.defineProperty,K=Object.getOwnPropertyDescriptor,h=(e,t,r,a)=>{for(var n=a>1?void 0:a?K(t,r):t,l=e.length-1,p;l>=0;l--)(p=e[l])&&(n=(a?p(t,r,n):p(n))||n);return a&&n&&F(t,r,n),n};let o=class extends M(P){async firstUpdated(){if(!this.peer&&!this.peerChatHash)throw new Error('peer-chat must be initialized with either the "peerChatHash" or the "peer" input.');if(this.peer&&!this.peerChatHash){const e=await b(this.store.peerChatsForPeer.get(this.peer));e.length===0?this.peerChatHash=await this.store.client.createPeerChat(this.peer):this.peerChatHash=e[0]}}renderTopBar(e){const t=e.peer_1.agents.find(r=>c(r)===c(this.store.client.client.myPubKey))?e.peer_2:e.peer_1;return s`
			<div
				part="top-bar"
				class="row top-bar"
			...
			<div part="chat" class="column" style="flex: 1; margin: 8px">
				<div class="flex-scrollable-parent">
					<div class="flex-scrollable-container">
						<div
							${E(i=>{if(!i)return;const u=[].concat(...f.map(d=>d.eventsSets)).filter(d=>!g.includes(c(d[0][1].provenance))),v=[];for(const d of u)for(const[x,k]of d)r.includes(x)||v.push(S(x));v.length>0&&this.store.peerChats.get(this.peerChatHash).markMessagesAsRead(v)})}
		...
		`}async sendMessage(e){try{await this.store.peerChats.get(this.peerChatHash).sendMessage(e)}catch(t){console.error(t),B(m("Error sending the message"))}}get peerChatStore(){return this.store.peerChats.get(this.peerChatHash)}chatInfo(){return this.peerChatHash?_([this.peerChatStore.currentPeerChat.get(),this.peerChatStore.messages.get(),this.peerChatStore.readMessages.get()]):{status:"pending"}}render(){const e=this.chatInfo();switch(e.status){case"pending":return s`
					<sl-skeleton
				...
				`;case"error":return s`<display-error
					...
					.error=${e.error}
				></display-error>`;case"completed":const[t,r,a]=e.value;return this.renderChat(t,r,a.myReadMessages)}}};o.styles=[T,H`
			:host {
				display: flex;
				font-size: 14px;
			}
		`];h([w(C("peer-chat-hash"))],o.prototype,"peerChatHash",2);h([w(C("peer"))],o.prototype,"peer",2);h([y({context:O,subscribe:!0})],o.prototype,"store",2);h([y({context:$,subscribe:!0})],o.prototype,"linkedDevicesStore",2);h([y({context:A,subscribe:!0})],o.prototype,"profilesStore",2);o=h([D(),j("peer-chat")],o);export{o as PeerChatEl};
